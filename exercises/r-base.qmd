...
integers. This is convenient but on large data volumes it can
...
_Doubles_ can also be written in scientific notation:
```{r}
#| echo: true
2e3
class(2e3)
...
a calculator. We can therefore do additions:
```{r}
#| echo: true
# Addition
8 + 9
...
Character *strings* are used to store textual information.
...
To create a character _string_, we
...
'it\'s the apostrophe, no problem' # <1>
```
...
Factors are used to represent __categorical variables__,
...
Here's a list of municipality codes from the [French Official Geographic Code]((https://www.insee.fr/fr/information/6800675))
```{r}
#| echo: true
municipality_list <- c(
